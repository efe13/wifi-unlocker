...
public class WifiNetwork implements Comparable<WifiNetwork>, Serializable{
	...
	static enum TYPE {
		...
		SKY_V1 , WLAN6 ,TECOM , INFOSTRADA, CABLEVISION };
	public WifiNetwork(String ssid, String mac, int level , String enc , Context con ){
		...
		//check if the mac huawei its  supported since you can change the ssid perhaps is still supported
	...
	private boolean essidFilter(Context con) {
		if ( ( ssid.startsWith("Thomson") && ssid.length() == 13 )    ||
		     ...
		     //( ssid.startsWith("Intercable") && ssid.length() == 14 ) commented until confirm
		     )
		{
		...
		/*if(ssid.matches("Intercable[0-9]{4}"))
		{
			ssidSubpart=ssid;
			type = TYPE.CABLEVISION;
			return true;
		}*/
